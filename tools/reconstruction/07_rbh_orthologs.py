...
These tables are used by
  * <work>/audits/common_network.py   run every species' enzyme parameters on the identical
...
Output  <work>/tables/rbh/pid_<species>.tsv     reference_gene, ortholog, pident (no header)
Run:    python3 tools/reconstruction/07_rbh_orthologs.py       (~2 min; needs diamond in PATH)
...
# Reference taxon and targets from the reconstruction config (was hard-coded). A taxon
# whose proteome is a UniProt TSV rather than a FASTA is written out below via
# load_proteome, keyed by its ordered-locus tags.
_CFG = config()
_REF = _CFG['scaffold_taxon']
REFERENCE = PROTEOME[_REF]
TARGETS = {sp: (PROTEOME[sp] if str(PROTEOME[sp]).endswith(('.faa', '.fasta', '.fa')) else None)
           for sp in _CFG['taxa'] if sp != _REF}
...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for sp, faa in TARGETS.items():
            ...
            ab = best_hits(REFERENCE, faa, td, f'ref2{sp}'); ba = best_hits(faa, REFERENCE, td, f'{sp}2ref')
