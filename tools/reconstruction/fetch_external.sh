...
# =============================================================================
#   bash tools/reconstruction/fetch_external.sh            everything (~9 GB, most of it KOfam profiles)
#   bash tools/reconstruction/fetch_external.sh --no-kofam skip KOfam (only needed to redo 04_kofam_annotate.sh;
#                                         the KO tables it produces are tracked in <work>/inputs/kofam/)
#
# Everything lands in <external>/, which git ignores. What is fetched and why:
#
...
#                      the protein FASTA the pipeline uses is tracked at <proteomes>/.
#
# Curated scaffold models are fetched by 01_fetch_curated_models.sh, not here.
# =============================================================================
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
. "$HERE/common.sh"
EXT="$RECON_EXTERNAL"; mkdir -p "$EXT"; cd "$EXT"
