...
carbon budget (not 20 free caps). Verified working on iDC1003 (parapsilosis).
    ...
    python3 tools/reconstruction/03_apply_medium_fba.py \\
        parapsilosis_iDC1003.xml medium_iDC1003_parapsilosis.csv

Needs: cobra, pandas. Writes <work>/tables/medium_fba_<model>.csv.
...
def main():
    ...
    ap.add_argument("model", help="SBML model (absolute, or a name under <work>/models/)")
    ap.add_argument("medium", help="medium map CSV (absolute, or a name under <work>/inputs/)")
    a = ap.parse_args()
    configure_from_args(a)
    model_path = a.model if Path(a.model).exists() else str(MODELS / a.model)
    med_path = a.medium if Path(a.medium).exists() else str(INPUTS / a.medium)
    ...
    out = TABLES / f"medium_fba_{Path(model_path).stem}.csv"
