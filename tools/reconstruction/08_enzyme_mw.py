...
Inputs
    <work>/models/<model>.xml                     gene ids in GPRs
    <proteomes>/auris_cladeI.faa           C. auris B8441 RefSeq (XP_ ids)
    <proteomes>/haemulonii.faa             C. haemulonii RefSeq (XP_ ids)
    <proteomes>/duobushaemulonii.faa       C. duobushaemulonii RefSeq (XP_ ids)
    <work>/inputs/parap_uniprot.tsv               C. parapsilosis CDC317 UniProt proteome
                                               ...
                                               the source.
Outputs
    <work>/tables/enzyme_mw_<species>.csv         gene, length_aa, MW_kDa

Run:  python3 tools/reconstruction/08_enzyme_mw.py            (~10 s; needs cobra, biopython)
