...
# =============================================================================
#   bash tools/reconstruction/14_run_seq2topt_seq2tm.sh          (~15 min on a laptop CPU for 3042 proteins)
#
...
# test RMSE 12.3 C) and model_tm_window=3_r2=0.76.pth (RMSE 7.6 C).
# tools/reconstruction/fetch_external.sh clones the code, downloads the checkpoints and
# the ESM2 weights.
#
# PORTED from the standalone Candida etcGEM (Candidas `gem/`); the only change is that the
# directories are configurable (common.sh). Predictors and settings are unchanged.
#
# Input   $RECON_TABLES/seq2topt_input.csv           from 13_seq2topt_input.py (id, sequence)
# Output  $RECON_TABLES/topt_pred.csv, tm_pred.csv   the predictors' raw output (id = row number)
#         $RECON_TABLES/thermal_topt.csv             id (taxon|gene), sequence, pred_topt
#         $RECON_TABLES/thermal_tm.csv               id (taxon|gene), sequence, pred_tm
#
...
# the standalone's gem/FIG4_LOCKED.md; measured in its gem/audits/padding_sensitivity.py).
# Re-running this
...
. "$HERE/common.sh"
S2T="$RECON_EXTERNAL/Seq2Topt"
TAB="$RECON_TABLES"
PY="${PYTHON:-python3}"
export TORCH_HOME="${TORCH_HOME:-$RECON_EXTERNAL/torch_home}"   # esm2_t6_8M_UR50D.pt lives in $TORCH_HOME/hub/checkpoints

[ -d "$S2T/code" ] || { echo "!! $S2T not found. Run tools/reconstruction/fetch_external.sh first."; exit 1; }
[ -f "$RECON_EXTERNAL/large_model_pth/model_topt_window=3_r2=0.57.pth" ] || { echo "!! checkpoints missing; run tools/reconstruction/fetch_external.sh"; exit 1; }
...
[ -e "$S2T/large_model_pth" ] || ln -s "$RECON_EXTERNAL/large_model_pth" "$S2T/large_model_pth"
...
EOF
echo "next: python3 tools/reconstruction/to_strain_inputs.py --species <taxon>"
echo "      (16-18 of the standalone are NOT ported: the model itself is src/etcgem)"
