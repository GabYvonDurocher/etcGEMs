...
```{python}
#| label: tbl-decomp
#| tbl-cap: "MCA decomposition of E_a^org (tuned model, rich medium). The control-weighted mean enzyme E_a, offset by allocation and maintenance, closes to the actual value (residual = higher-order / window nonlinearity)."
#| output: asis
import json, pandas as pd
D = json.load(open("assets/tables/ea_summary.json"))["headline_BHI"]["decomposition_eV"]
rows = [("control-weighted mean $E_{a,i}$ (enzyme-kinetic)", D["kinetic_control_weighted_mean_Ea_i"]),
        ("allocation (growth law)", D["allocation_growth_law"]),
        ("maintenance NGAM(T)", D["maintenance_NGAM"]),
        ("residual (nonlinear)", D["residual_nonlinear"]),
        ("**= $E_a^\\text{org}$ (actual)**", D["actual_Ea_org"])]
