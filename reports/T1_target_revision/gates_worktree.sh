#!/bin/zsh
# T1 -- the seven-strain gate battery, run IN THE T1 WORKTREE so the option under test is the
# code exercised. Every exit code printed. CANDIDAS_ROOT unset.
cd "/Users/g.yvon-durocher/Library/CloudStorage/OneDrive-UniversityofExeter/Documents/work/MICROADAPT/etcGEMs-t1"
unset CANDIDAS_ROOT
...
$P reports/candida_thermal_limit/gate_table.py > /tmp/t1_gate_table.out 2>&1; echo "gate_table rc=$?"; grep -E "comparisons" /tmp/t1_gate_table.out | tail -1
$P reports/P1_parsa_port/gate.py > /tmp/t1_gate_p1.out 2>&1; echo "P1 gate rc=$?"; grep -E "comparisons" /tmp/t1_gate_p1.out | tail -1
