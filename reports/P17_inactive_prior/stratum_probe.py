"""Local kernel diagnostic within the algebraically identified P16 stratum."""
...
def main():
    ...
    for tag,it in [('red1',8000),('red2',8800)]:
        ...
        for seed in [17201,17202,17203]:
            ...
            rng=np.random.default_rng(seed);u=r.samples_u[chosen].copy();u[7]=rng.random();bound=r.bound[r.bound_iter[it]];axes=bound.get_random_axes(rng);scale=float(r.scale[it]);cut=float(r.logl[it-1]);t=time.monotonic();calls=0
            def ll(theta):
                nonlocal calls
            ...
            record=dict(tag=tag,iteration=it,seed=seed,stored_index=chosen,group_count=len(group),cut=cut,initial_logl=initial,stored_logl=float(r.logl[chosen]),initial_flux=initial_flux,start=u.tolist(),scale=scale,axes_row_norm=np.linalg.norm(axes,axis=1).tolist())
            try:
            ...
            record.update(calls=calls,wall_s=time.monotonic()-t);rows.append(record);(a.HERE/'stratum_probe.json').write_text(json.dumps(rows,indent=2));print(json.dumps({k:v for k,v in record.items() if k not in ['initial_flux','axes_row_norm','start','end']}),flush=True)
