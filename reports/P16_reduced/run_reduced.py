#!/usr/bin/env python3
"""P16 TASK 4 -- the REDUCED nested run: 15 free parameters, dTm pinned at 0.0 (D2).

P15's runner with exactly one change beyond the output directory: the sampler is handed the 15
free specs and `reduced.loglike15`, which re-inserts dTm = 0 before calling the core likelihood.
Settings are P15 D1's EXACTLY except the dimension -- same nlive, same rslice, same slices, same
dlogz, same first_update, same checkpoint interval. The sampler is NOT changed, in particular not
in response to P15's crash.

ORIGINAL HEADER FOLLOWS.

P15 TASK 1 -- nested sampling of the D NLDM posterior with dynesty, at nlive 800.

P11's runner unchanged except for three things, all stated: the output directory is P15's, the
sampler is constructed with `first_update={'min_eff': 30}` (P11 D4 measured ~1 h of single-core
unit-cube phase without it), and the progress log lives beside this file. The likelihood, the
prior transform, the pool path and the stopping logic are P11's exactly, so the two runs differ
from P11's only in nlive, the seed, and the clamp support form that P13 committed to the strain
config.

ORIGINAL P11 HEADER FOLLOWS.
...
sys.path.insert(0, os.path.join(ROOT, "reports", "P11_nested"))   # prior_transform
os.chdir(ROOT)
...
from reduced import FREE_SPECS, FREE_NAMES, FIXED_NAME, FIXED_SAMPLED, loglike15                     # noqa: E402
...
def main():
    ...
    ap.add_argument("--tag", default="main"); ap.add_argument("--nlive", type=int, default=800)
    ...
    ap.add_argument("--hours", type=float, default=16.0); ap.add_argument("--until", default=None,
    ...
    a = ap.parse_args()
    import dynesty
    specs = FREE_SPECS; D = len(specs)          # P16: 15 free, dTm pinned
    ptform = transform_factory(specs)
    print(f"[nested] REDUCED: {D} free parameters; {FIXED_NAME} pinned at {FIXED_SAMPLED} "
          f"(sampled space). Free: {FREE_NAMES}", flush=True)
    assert D == 15 and FIXED_NAME not in FREE_NAMES
    ...
    assert str(ctx["respiration"].get("support")) == "clamp", ctx["respiration"]   # P13's form
    ...
    with Pool(a.nproc, initializer=_gwinit, initargs=(dict(PAYLOAD, solver=solver),)) as pool:
        if a.resume and os.path.exists(ckpt):
            ...
            s = dynesty.NestedSampler(loglike15, ptform, D, nlive=a.nlive, bound=a.bound, sample=a.sample,
                                      slices=a.slices, pool=pool, queue_size=a.nproc,
                                      first_update={'min_eff': 30},   # P11 D4: skips ~1 h of
                                      # single-core unit-cube sampling before the first bound
    ...
    summ = dict(tag=a.tag, fixed_parameter=FIXED_NAME, fixed_value=FIXED_SAMPLED, n_free=D, converged_on_dlogz=bool(converged), hours_cap=a.hours, nlive=a.nlive, sample=a.sample, slices=a.slices, bound=a.bound, seed=a.seed, nproc=a.nproc,
