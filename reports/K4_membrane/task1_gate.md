...
## 3a. What the chain is actually for: pyrimidines, not energy

Deleting any single complex is harmless, which looks like respiratory redundancy. It is not.
Deleting **complex II together with either quinol oxidase** is lethal in all four species, and
in no other combination:

| deletion (40 °C) | *C. auris* | *C. parapsilosis* |
|---|---|---|
| any one of I, II, III, IV | 100 % of base | 48–100 % |
| III + IV | 100 % | lethal via cI+cII+cIII/IV only |
| **II + III**, **II + IV** | **lethal** | lethal with I as well |

Tracing the ubiquinol balance in *C. auris* at 40 °C explains it. Ubiquinol has exactly one
producer and two consumers:

| flux (mmol gDW⁻¹ h⁻¹) | reaction | |
|---|---|---|
| **+0.005951** | `R01868__cyto` dihydroorotate dehydrogenase | **the only producer** |
| −0.005433 | `R02161__mito` complex III | consumer |
| −0.000517 | `R08657__mito` glycerol-3-phosphate dehydrogenase | consumer |

Dihydroorotate dehydrogenase is the pyrimidine-biosynthesis step, and it is itself essential —
deleting it gives zero growth. It reduces ubiquinone to ubiquinol, and that ubiquinol has to go
somewhere. Complex III is one sink; complex II **run backwards**, as fumarate reductase, is the
other. Remove both and pyrimidine synthesis stalls, so the cell cannot make RNA.

**So the respiratory chain's only essential role in these three models is to re-oxidise
0.006 mmol/gDW/h of ubiquinol generated by nucleotide biosynthesis.** Its flux is set by
pyrimidine demand, not by energy demand — which is why it sits at 0.005 while ATP synthase sits
at 10, and why an area budget over it changes nothing. This also means that "delete the
complexes and growth is zero" must not be read as evidence that respiration is
bioenergetically load-bearing here. It is not; the lethality is biosynthetic.
