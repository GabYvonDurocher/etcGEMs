...
applied mechanically. Writes task2b_checkpoints.csv and task2b_verdict.json beside this file.
"""
import json
import os
import sys
...
def main():
    ch = np.load(os.path.join(OUT, "chain.npy")); N, W, D = ch.shape
    ...
    for n in range(250, N + 1, 250):
        ...
        inc = tm - prev; prev = tm
        z = zck.get(n, {})
        rows.append(dict(N=n, sampler="zeus", walkers=W, tau_max=round(tm, 1), carrier=names[k], increment=round(inc, 1),
    ...
    df = pd.DataFrame(rows); df.to_csv(os.path.join(HERE, "task2b_checkpoints.csv"), index=False)
    ...
    json.dump(v, open(os.path.join(HERE, "task2b_verdict.json"), "w"), indent=2)
