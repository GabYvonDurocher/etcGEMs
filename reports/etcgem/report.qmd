...
predicted reference curve that the validation section then compares against measured *E. coli*
curves.
...
real data it never saw. We compare the model's predicted growth curves — **with nothing fit
to growth** — against 26 measured *E. coli* growth curves, one prediction per curve under that
experiment's medium, and separately check whether the model's predicted enzyme demands track a
measured proteome.
...
0.96 eV on minimal, versus observed rising-limb values ~0.64–0.73 eV. Because $E_a$ here is
an emergent output of the literature MMRT curvature prior rather than a fitted target, this
agreement is a genuine test rather than a calibration. A residual gap remains
...
temperature grid. They are *not* an average over the 26 empirical curves; those enter only in
