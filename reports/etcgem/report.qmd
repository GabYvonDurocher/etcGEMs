...
abstract: |
  ...
  **measured temperature-dependent** sector allocation (chaperone/stress sector ramping
  ...
  curve** against 26 *E. coli* growth TPCs (Smith 2019 database), the emergent model
  predicts curve shape with a median $R^2=0.59$ (range 0.04–0.94), reproducing the
  optimum and the denaturation-set upper limit (~47 °C) but over-predicting the low-T
  rising limb and getting magnitude only order-of-magnitude right — honest a-priori
  errors, not tuning failures. Its predicted per-enzyme allocation correlates with the
  measured proteome ($\rho\approx0.3$). On this model the widely-assumed clean
...
**Per-curve TPC prediction.** Rather than one pooled fit, we score the emergent
prediction against **each of 26 individual *E. coli* growth TPCs** from the Smith 2019
database [@Smith2019] (the medium set as availability — glucose minimal aerobic, the
model default, matching the two defined-medium curves; uptake not pinned). The
distribution of per-curve skill (@fig-percurveR2, @fig-percurve, @tbl-percurve) has a
**median shape $R^2=0.59$ (range 0.04–0.94)**: the emergent model predicts some curves
well and others poorly. It reproduces the optimum ($T_\text{opt}=37$ °C) and the
**denaturation-set upper limit** ($CT_\text{max}\approx47$ °C, matching where the
empirical curves collapse), but the emergent rising limb is **too steep** — it
over-predicts low-temperature growth and peaks ~3–5 °C colder than observed — and the
emergent magnitude is only order-of-magnitude right (predicted $r_\text{max}=0.34$ vs
~0.1–1.4 $\text{h}^{-1}$ across the defined-medium curves). These are honest a-priori
discrepancies — pointing to missing cold-side physiology and to $k_\text{cat}$/$\sigma$
uncertainty — not tuning failures. On the **independent** activation-energy test, the
emergent $E_a\approx0.95$ eV sits just above the mesophilic-bacterial-growth benchmark
of 0.84–0.88 eV [@Smith2019], and far from the 0.65 eV metabolic-theory value we
previously (wrongly) calibrated to. The nominal descriptors are in @tbl-nominal.

![**Emergent model: distribution of per-curve skill.** Per-curve shape $R^2$ across 26 individual *E. coli* TPCs, with nothing fit to growth.](assets/figures/percurve_R2_distribution.png){#fig-percurveR2 fig-pos="H" width=70%}

![**Emergent model (red) vs individual *E. coli* TPCs (grey, peak-normalised).** Twelve example curves; the model captures the optimum and collapse but over-predicts the low-temperature rising limb.](assets/figures/percurve_smallmultiples.png){#fig-percurve fig-pos="H" width=95%}

```{python}
#| label: tbl-percurve
#| tbl-cap: "Per-curve validation summary: distribution of shape R² across the 26 emergent-model vs individual E. coli TPC comparisons, and the emergent descriptors. Nothing is fit to growth."
#| output: asis
import json, pandas as pd
with open("assets/tables/percurve_summary.json") as fh:
    s = json.load(fh)
rows = [("curves validated", s["n_curves"]),
        ("defined-medium curves", s["n_defined"]),
        ("median shape R²", s["shape_R2_median"]),
        ("mean shape R²", s["shape_R2_mean"]),
        ("curves with shape R² ≥ 0.5", s.get("shape_R2_ge_0.5", "—")),
        ("emergent T_opt (°C)", s["emergent"]["Topt_C"]),
        ("emergent r_max (1/h)", s["emergent"]["rmax_per_h"]),
        ("emergent CT_max (°C)", s["emergent"]["CTmax_C"]),
        ("emergent E_a (eV)", s["emergent"]["Ea_eV"])]
print(pd.DataFrame(rows, columns=["quantity", "value"]).to_markdown(index=False))
```

```{python}
#| label: tbl-nominal
#| tbl-cap: "Emergent nominal TPC descriptors of the complete model (glucose minimal aerobic; nothing fit to growth)."
#| output: asis
import json
import pandas as pd

with open("assets/tables/percurve_summary.json") as fh:
    em = json.load(fh)["emergent"]
rows = [("T_opt (°C)", em["Topt_C"]), ("r_max (1/h)", em["rmax_per_h"]),
        ("CT_max (°C)", em["CTmax_C"]), ("E_a (eV)", em["Ea_eV"])]
df = pd.DataFrame(rows, columns=["Descriptor", "Emergent"])
...
h⁻¹) is only order-of-magnitude right, reflecting genuine uncertainty in $\sigma$ and in
in-vivo $k_\text{cat}$ (carried as literature ranges, not tuned); **(iii)** the
...
![**Model-construction ablations.** Nominal TPC of each layer combination over the empirical cloud; unfolding ($T_m$) is the decisive layer.](assets/figures/ablation_comparison.png){#fig-ablation fig-pos="H" width=80%}
