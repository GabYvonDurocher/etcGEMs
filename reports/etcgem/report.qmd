...
abstract: |
  ...
  against two high-quality, **strain-matched** curves — a defined glucose-minimal curve (K-12
  NCM3722, Noll 2023, with per-temperature SD) and a rich LB curve (MG1655 wild-type, Erdos
  2026) — each predicted a priori under its medium (availability, uptake unpinned), the emergent
  model **reproduces the growth-law magnitude jump between media**: a predicted rich/minimal
  $r_\text{max}$ ratio of 1.9× (1.04 h⁻¹ LB vs 0.55 h⁻¹ minimal, straight from the measured LB
  ribosome fraction) against an observed ~2.1×. It does *not* reproduce the absolute level —
  both peaks are under-predicted about two-fold (minimal 0.55 vs 1.13; rich 1.04 vs 2.35 h⁻¹) —
  and on minimal the optimum is ~12 °C too cold; on rich the optimum and activation energy are
  close ($E_a$ 0.83 eV vs observed 0.76, near the ~0.85 eV benchmark). These are honest a-priori
  results — the magnitude shortfall points to $\sigma$/$k_\text{cat}$/medium uncertainty — not
  tuning failures. Its predicted
...
| validation curves | strain-matched TPCs | glucose-minimal K-12 NCM3722 [@Noll2023]; LB MG1655 wt [@Erdos2026]; LB medium def. from MRes/Machado [@Madkaikar2023; @Machado2021] | 2 curves |
...
**Validation.** The model is validated against two trusted, strain-matched *E. coli* growth
curves — a defined glucose-minimal curve (K-12 NCM3722; Noll 2023 [@Noll2023], with a
per-temperature SD) and a rich LB curve (K-12 MG1655 wild-type; Erdos 2026 [@Erdos2026],
figure-digitized) — each predicted a priori under its medium (availability, uptake unpinned)
with its medium-matched sector fractions. The **primary metric is absolute** $R^2$/RMSE on raw
specific growth rate ($\text{h}^{-1}$); the rising-limb $E_a$ (tested against the ~0.85 eV
mesophilic-growth benchmark) is a secondary diagnostic. The proteome is validated separately
(predicted vs measured per-enzyme mass, §Validation).

# How the model works: the reference operating point and its enzyme-level parameters
...
**reference operating point** we use throughout the rest of the report, shows the growth curve
the model predicts there, and displays the per-enzyme parameters underneath that curve. The
next section then tests this reference prediction against real data.

**The reference operating point.** Everything below, and every sensitivity result later, is
computed at one condition: the **glucose-minimal strain nominal**. The organism grows on a
defined glucose-minimal medium, and the proteome budget is split using the measured
glucose-minimal sector fractions $f_\text{metab}=0.483$ (metabolic enzymes),
$f_\text{bio}=0.191$ (biosynthesis/ribosomes) and $f_\text{maint}=0.326$ (maintenance).
@fig-reftpc shows the growth curve the model predicts there, on absolute rate. Its descriptors
read off directly: the optimum is $T_\text{opt}=30$ °C, the maximum growth rate
$r_\text{max}=0.55\,\text{h}^{-1}$, the cold and hot limits $CT_\text{min}=9$ °C and
$CT_\text{max}=47$ °C, and the rising-limb activation energy $E_a\approx0.98$ eV. This is the
predicted reference curve that the validation section then compares against the measured
strain-matched glucose-minimal curve (Noll 2023, K-12 NCM3722).

![**Reference growth thermal performance curve.** The model's predicted growth rate versus temperature at the reference operating point (glucose-minimal strain nominal; sector fractions shown), on raw absolute rate (h⁻¹). The optimum $T_\text{opt}$, maximum rate $r_\text{max}$, cold and hot limits $CT_\text{min}$/$CT_\text{max}$ (where growth falls to 5 % of peak) and the rising-limb activation energy $E_a$ are marked.](assets/figures/reference_tpc.png){#fig-reftpc fig-pos="H" width=85%}
...
deviations are worth noting because the sensitivity analyses later use exactly these spreads
...
The sensitivity and decomposition analyses later in the report do not tune these 2,560 enzymes
individually; instead they move the whole distributions in @fig-enzdens with a few global dials
(described in @sec-sensitivity), while keeping each enzyme's grounded value relative to the
rest.

# Validation against two trusted, strain-matched curves

**What this section does.** The strongest test of a forward model is whether it reproduces
real data it never saw. We validate the model — **with nothing fit to growth** — against two
high-quality, strain-matched *E. coli* growth curves: a **defined glucose-minimal** curve
(K-12 NCM3722; Noll / Katipoglu-Yazan et al. 2023 [@Noll2023], with a per-temperature standard
deviation) and a **rich LB** curve (K-12 MG1655 wild-type; Erdos et al. 2026 [@Erdos2026]).
Each is predicted a priori under its own medium, and we compare on **raw absolute growth rate
(h⁻¹)**. The headline is the minimal-vs-rich magnitude contrast: does the measured, medium-matched
proteome allocation reproduce the observed jump from minimal to rich growth?

**Data provenance — why the validation set changed.** Earlier drafts validated against a
larger compilation of literature *E. coli* curves (Smith et al. 2019). That compilation is no
longer trusted: one curve was found to be mis-scaled by roughly 8-fold, and for most curves the
exact strain, medium and data robustness are uncertain. We therefore moved to two strain-matched
primary sources where the organism, the medium and the measurement are documented. Their
limitations are stated honestly: the Noll minimal curve is transcribed from the paper's table
with a per-temperature SD over 28–40 replicate wells (the quantitative anchor); the Erdos rich
curve was **digitized by eye** from a published figure (unpublished data, precision ≈ ±0.1 h⁻¹)
and is treated as the lower-precision rich-medium comparator. An independent rich-broth curve
(Van Derlinden et al. 2012, BHI) is available as an optional cross-check.

**Medium as an input.** The medium enters as *availability*, not as a pinned uptake rate: we
open the exchange uptakes (`EX_*_REV`) of the medium's components and let the enzyme-constrained
model determine actual uptake. Noll is predicted under **glucose-minimal**, Erdos under an
**LB** medium (a GEM-compatible definition of 62 amino-acid / nucleoside / vitamin / ion
components). The minimal medium also carries six trace amino acids; opening those uptakes as a
sensitivity check leaves the predicted minimal $r_\text{max}$ essentially unchanged.

**Result — the model reproduces the medium contrast but under-predicts both absolute rates.**
The central finding (@fig-valcurves, @fig-valcontrast, @tbl-val) is that the **medium-matched
sector allocation reproduces the observed rich-over-minimal magnitude jump**: the model
predicts $r_\text{max}$ = 1.04 h⁻¹ on LB versus 0.55 h⁻¹ on glucose-minimal, a **1.9-fold
ratio that emerges directly from the higher measured ribosome fraction on LB** — matching the
observed 2.1-fold ratio (Erdos 2.35 vs Noll 1.13 h⁻¹). What the model does *not* get is the
absolute level: it under-predicts both peaks by roughly two-fold (minimal 0.55 vs 1.13; rich
1.04 vs 2.35 h⁻¹), so the absolute-rate $R^2$ is negative. On the rich curve the optimum and
activation energy are close (predicted $T_\text{opt}$ 37 °C vs observed 40 °C; $E_a$ 0.83 eV vs
observed 0.76 eV, both near the ~0.85 eV mesophilic-bacterial-growth benchmark); on the minimal
curve the predicted optimum is ~12 °C too cold (30 vs 42 °C) and the rising limb too steep.
These are honest a-priori residuals — the two-fold magnitude shortfall points to the in-vivo
$k_\text{cat}$ scale, the saturation $\sigma$, or the exact medium composition — none of which
is tuned here. The per-curve numbers are in @tbl-val.

![**Emergent model vs the two trusted strain-matched curves (raw absolute rate, nothing fit).** Left: defined glucose-minimal (Noll 2023, K-12 NCM3722) with per-temperature mean ± SD error bars. Right: rich LB (Erdos 2026, MG1655 wt, figure-digitized). The model line (coloured) sits about two-fold below both peaks; absolute $R^2$/RMSE and observed-vs-predicted $r_\text{max}$ are annotated.](assets/figures/validation_trusted_curves.png){#fig-valcurves fig-pos="H" width=100%}

![**Minimal-vs-rich magnitude contrast.** Both curves on one axis: the minimal data (Noll, green, ±SD) sit near the *rich* model line, illustrating the ~2× absolute under-prediction, while the rich data (Erdos, orange) sit far above. The model reproduces the observed rich/minimal $r_\text{max}$ ratio (model 1.9× vs observed 2.1×) but not the absolute level.](assets/figures/validation_minimal_vs_rich.png){#fig-valcontrast fig-pos="H" width=80%}

```{python}
#| label: tbl-val
#| tbl-cap: "Validation against the two trusted strain-matched curves (nothing fit to growth). Absolute-rate fit and observed-vs-predicted descriptors per curve; the model's rich/minimal rmax ratio matches the observed contrast while both absolute rates are under-predicted ~2×."
#| output: asis
import json, pandas as pd
with open("assets/tables/validation_trusted_summary.json") as fh:
    s = json.load(fh)
mn, rh, mc = s["noll_minimal"], s["erdos_LB"], s["magnitude_contrast"]
def row(lab, k):
    return (lab, mn[k], rh[k])
rows = [("medium", "glucose-minimal", "LB (rich)"),
        ("strain", "K-12 NCM3722", "K-12 MG1655 wt"),
        ("n temperatures", mn["n"], rh["n"]),
        ("absolute R²", mn["abs_R2"], rh["abs_R2"]),
        ("RMSE (h⁻¹)", mn["RMSE_per_h"], rh["RMSE_per_h"]),
        ("observed r_max (h⁻¹)", mn["obs_rmax"], rh["obs_rmax"]),
        ("predicted r_max (h⁻¹)", mn["pred_rmax"], rh["pred_rmax"]),
        ("observed T_opt (°C)", mn["obs_Topt_C"], rh["obs_Topt_C"]),
        ("predicted T_opt (°C)", mn["pred_Topt_C"], rh["pred_Topt_C"]),
        ("observed E_a (eV)", mn["obs_Ea_eV"], rh["obs_Ea_eV"]),
        ("predicted E_a (eV)", mn["pred_Ea_eV"], rh["pred_Ea_eV"])]
print(pd.DataFrame(rows, columns=["quantity", "minimal (Noll)", "rich (Erdos)"]).to_markdown(index=False))
```
...
moves the *whole* per-enzyme distribution shown in @fig-enzdens while preserving its shape:
...
heterogeneity in @fig-enzkcat is preserved throughout, but not individually tuned. (The
per-enzyme *control* analysis later does act enzyme-by-enzyme, and is the complementary
fine-grained view.)

**Everything here is computed on the single reference curve.** These elasticities — and the
decomposition in the next section — are computed on **one** model growth curve at the reference
operating point (the glucose-minimal strain nominal of @fig-reftpc), evaluated over the
temperature grid. They are *not* an average over the empirical curves; those enter only in
...
distributions** (@fig-enzdens) rather than by tuning individual enzymes, and the whole analysis
runs on the **single reference growth curve** (glucose-minimal, @fig-reftpc) over the
...
Remaining limitations, all structural / in-silico, and now framed by the trusted-curve
validation: **(i)** on the minimal curve the emergent rising limb is **too steep** and the
optimum **~12 °C too cold** (predicted 30 °C vs observed 42 °C); a single literature $\Delta
C_p$ prior couples breadth and $E_a$, so per-enzyme curvature (from broader DLTKcat coverage)
is the route to a more accurate rising limb and optimum; **(ii)** the **magnitude** captures
the medium contrast (predicted rich/minimal ratio 1.9× vs observed ~2.1×) but **under-predicts
both absolute rates about two-fold** (minimal 0.55 vs 1.13; rich 1.04 vs 2.35 h⁻¹), reflecting
residual uncertainty in $\sigma$, in-vivo $k_\text{cat}$ and exact medium composition (all
carried as literature values/availability, not tuned); **(iii)** the
...
The complete model above is one configuration. To show how much each layer contributes, we
build it up one layer at a time and score each version against the empirical glucose-minimal
growth curves (@fig-ablation, @tbl-ablation). Each row adds one ingredient to the row above.
