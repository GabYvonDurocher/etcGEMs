...
abstract: |
  ...
  **measured temperature-dependent** sector allocation (chaperone/stress sector ramping
  2.7-fold from 30→43 °C). Validated against 26 empirical *E. coli* growth-TPC datasets
  it reproduces the optimum (37 °C vs 40 °C observed) and the denaturation-set upper
  limit (~47 °C), and its predicted per-enzyme allocation correlates with measured
  abundance ($\rho\approx0.3$). On this complete model the widely-assumed clean
...
The complete model builds and grows (translation cap and pool co-limiting at the
nominal split, $\mu^\ast=0.43\,\text{h}^{-1}$ at 30 °C).

# Validation

We lead with whether the complete model **reproduces the data**.

**Empirical growth TPC.** @fig-emptpc overlays the model's nominal TPC on 566
peak-normalised growth-rate measurements from 26 literature *E. coli* datasets. The
model reproduces the position of the optimum (**$T_\text{opt}=37$ °C** vs an empirical
median peak of 40 °C) and, importantly, the **sharp denaturation-set upper limit**
(**$CT_\text{max}\approx47$ °C**) that matches where the empirical curves collapse; the
peak-normalised shape $R^2=0.47$ (binned-median $R^2=0.55$), the main discrepancy being a
rising limb steeper than observed ($E_a=0.87$ eV vs the conserved ~0.65 eV). The nominal
descriptors are in @tbl-nominal.

![**Complete model vs empirical *E. coli* growth TPC.** Model nominal curve (red) over 566 peak-normalised measurements from 26 datasets; the optimum (~37 °C) and the denaturation collapse (~47 °C) match the data, with a steeper-than-observed rising limb.](assets/figures/complete_vs_empirical_tpc.png){#fig-emptpc fig-pos="H" width=80%}

```{python}
#| label: tbl-nominal
#| tbl-cap: "Nominal TPC descriptors of the complete model."
#| output: asis
import json
import pandas as pd
...
rows = [("T_opt (°C)", nom["Topt_C"]), ("r_max (1/h)", nom["rmax"]),
        ("CT_min (°C)", nom["CTmin_C"]), ("CT_max (°C)", nom["CTmax_C"]),
        ("Niche width (°C)", nom["niche_width_C"]), ("E_a (eV)", nom["Ea_eV"])]
...
shift ($\rho=0.73$); $CT_\text{max}$ and breadth track $dT_\text{opt}$ through the fixed
denaturation ceiling ($\rho=0.78$, $-0.64$); $E_a$ tracks the curvature (`dCp_scale`
$\rho=0.65$); and — tellingly — **$r_\text{max}$ is depressed by envelope shifts**
($dT_\text{opt}$ $\rho=-0.60$) more than it is raised by the pool budget ($\rho=0.33$),
...
@fig-decompach shows the ranges reachable by each axis alone.
...
Remaining limitations, all structural / in-silico: **(i)** the rising-limb $E_a$
(~0.87 eV) is steeper than the conserved ~0.65 eV — a single per-enzyme $\Delta C_p$
couples breadth and $E_a$, so per-enzyme curvature calibration is still needed;
**(ii)** the chaperone/stress sector is imposed as a maintenance input, not modelled as
flux-carrying reactions; **(iii)** identifiability is a first-order proxy, not a
Fisher-information analysis; **(iv)** parameters and proteome are *E. coli*-specific
(per-strain proteomes / predictors are the library-scale route); **(v)** growth only —
respiration and carbon-use efficiency are future work [@Machado2021].
