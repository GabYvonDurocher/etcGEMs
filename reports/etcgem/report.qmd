...
abstract: |
  ...
  **per individual
  ...
  per-enzyme allocation correlates with the measured proteome ($\rho\approx0.3$). On this
  model the widely-assumed clean
...
uses a crossed two-group functional ANOVA with an exact Shapley split
$\varphi_A=S_A+\tfrac12 S_{AE}$, $\varphi_E=S_E+\tfrac12 S_{AE}$; per-enzyme thermal
control coefficients are central finite differences of each descriptor to each enzyme's
$T_{\text{opt},i}$/$\Delta C_{p,i}$, and identifiability is the max-normalised
...
TPC to descriptors and Spearman indices. (O4) The **decomposition** evaluates a crossed
$16\times16$ allocation$\times$envelope design (allocation = sector split around the
measured nominal; envelope = grounded $T_\text{opt}/T_m$ perturbations) and applies the
Shapley split of @eq-alloc. (O2/O5) The **control/identifiability** analysis screens all
...
additive optimum shift $dT_\text{opt}$ (nominal 0) moves by $\pm h\,\Delta_\text{ref}$
with $\Delta_\text{ref}=4.25$ K, the standard deviation of the per-enzyme $T_\text{opt}$
distribution, so it is "$h$ of a natural temperature scale"; sector fractions move by
$\text{nominal}\times(1\pm h)$ with the biosynthesis sector renormalised. Because every
input uses the same $h$, the elasticities are directly comparable in magnitude
(@fig-elast-tornado, @fig-elast-heat, @tbl-elasticity).

**Result — allocation drives the rate, the envelope drives the shape, the limit is
frozen.** The ranking is clear and, in places, sharper than the rank-correlation view:
...
- **$E_a$ is driven by the curvature and the allocation** (`dCp_scale` $E=+0.53$,
  $f_\text{metab}$ $+1.3$, `budget_scale` $+0.51$).
- **$CT_\text{max}$ is essentially insensitive to *every* swept input** ($|E|\le0.01$):
  it is pinned to the *fixed, grounded* melting temperatures, so nothing we perturb here
  moves it — the only lever on $CT_\text{max}$ is $T_m$ itself.
- **Thermal breadth** is envelope-driven (`dCp_scale` $E=-0.41$, `topt_scale` $-0.39$),
  acting through the cold limit $CT_\text{min}$ rather than the frozen $CT_\text{max}$.

This differs from the rank-correlation ranking (@fig-sens) mainly because that view swept
inputs over *unequal* ranges and scored monotonic consistency, so a wide `dCp_scale`
swing and the peak's drift toward $T_m$ over a wide $dT_\text{opt}$ range register there,
whereas the equal-step *local* elasticities isolate the structural leverage — chiefly the
allocation split for the rate and the (unperturbable) $T_m$ for the upper limit. These
elasticities answer the **structural** question ("which parts of the model drive the
TPC"); a complementary **uncertainty-weighted** analysis — varying each input across its
real measurement uncertainty to prioritise *what to measure* — is a planned follow-up.

![**Equal-perturbation elasticities per descriptor.** Inputs ranked by $|E|$ for each TPC descriptor (red $+$, blue $-$); note $CT_\text{max}$ is near-zero for every input (frozen at the grounded $T_m$).](assets/figures/elasticity_tornado.png){#fig-elast-tornado fig-pos="H" width=95%}
...
#| tbl-cap: "Standardised elasticities E[D,p] on the complete model (equal step h=0.10; additive dTopt by ±h·Δref, Δref=4.25 K = sd of enzyme Topt). Directly comparable in magnitude across inputs."
#| output: asis
import pandas as pd
