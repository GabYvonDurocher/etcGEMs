...
abstract: |
  ...
  *allocation*. The nominal model reproduces a realistic *E. coli* optimum
  ...
  model yields an interior growth optimum at an intermediate metabolic allocation. We
  emphasise that this apparent division of labour is partly a consequence of model
  structure (peak-normalised $k_\text{cat}$ and a temperature-independent allocation
  budget) and is not resolvable from growth data alone: distinguishing a genome-set
  envelope from an allocation-set magnitude will require temperature-resolved proteome
  and flux calibration together with a temperature-dependent allocation model. All
...
Importantly, this clean division of labour is **partly structural** — a property of
the modelling framework — and not a purely emergent biological result. That
$r_\text{max}$ is ~100 % allocation is largely *by construction*: peak-normalisation
fixes each enzyme's reference $k_\text{cat}$ to its own maximum, so at the organismal
optimum enzyme cost is, to first order, independent of the thermal envelope and simply
scales with the proteome budget; and the allocation axis is temperature-*independent*,
so it cannot by itself reshape the temperature response. The "envelope sets shape,
allocation sets magnitude" split is therefore in part a consequence of these two
modelling choices. Fitting the model to an empirical growth TPC would calibrate the
emergent curve but would **not** resolve the envelope-versus-allocation partition,
which is under-determined from growth data alone [@Pettersen2023]; resolving it
requires data that observe the internal state — temperature-resolved proteomics (for
allocation) and $^{13}$C fluxomics (for in-vivo $k_\text{cat}$) across multiple
temperatures. Moreover, real proteome allocation is itself temperature-dependent:
coarse-grained proteome-allocation models of temperature-dependent growth show
reallocation toward chaperone / stress-response proteins at temperature extremes
[@Mairet2021; @Wang2026], so a temperature-dependent chaperone/stress sector,
calibrated to proteomics, would let allocation contribute to curve *shape* and make
the partition identifiable. The decomposition should thus be read as operationalising
the *measurement* of the genome-set (envelope) versus allocation-set (magnitude) split
— the informative quantities being the interaction terms and the across-genome
variation in the split — rather than as evidence that the separation is a fixed
biological law.
