...
_Started 2026-09-08 while P2 was running; last updated 2026-09-08 by K4._ This is the standing list of what is outstanding across
...
| 3.8 | **Repair the mitochondrial proton accounting in the three iRV973-derived Candida models** — eleven uncosted, reversible metabolite/H⁺ symporters close the proton circuit outside the ETC, so the respiratory chain supplies **0.03 %** of the protons ATP synthase consumes and complex I–IV knockouts move growth by <0.006 % (K4 TASK 1). Stripping only the proton coupling makes the chain carry the whole load and costs ~⅓ of predicted growth. **The defect is in the published iRV973, not in the K1 port** (checked read-only). | Before ANY mechanism that acts on respiration is applied to these three species — a membrane-area budget, proton leak, an AOX branch. It is free, needs no new data, and blocks all of them. Also send to Ilgaz: it is his reconstruction's behaviour. |
| 3.9 | **A fifth sink-audit class: uncosted transport of the coupling ion.** `etcgem audit-sinks` looks for uncosted movement of ATP, NAD(P)H, FADH₂, quinol, ferredoxin (classes A–C) or a terminal electron acceptor (class D). It does not look for uncosted transport of H⁺ (or Na⁺) across an energy-transducing membrane, so all eleven carriers in 3.8 pass the audit clean — in models where the audit already reports 44–47 class-A hits. | When the audit is next touched. The Candida case is the worked example and the fix is one class. Until then the audit's silence about the proton circuit should not be read as a clean bill. |
