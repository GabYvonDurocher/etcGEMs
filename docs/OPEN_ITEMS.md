...
| 1.17 | **Run the remaining eight gas-flux fits under the new likelihood, or not** | PI | P11 converged configuration D on NLDM in 4.8 h and 111,420 evaluations. Projected at the same evaluation count and P10's per-fit tie-break costs: **D LB 4.5 h, D M9 4.8, E NLDM 4.7, E LB 5.0, E M9 4.8, F NLDM 4.8, F M9 4.8 — about 33 h in total**; **F LB is HELD** (P10 D1: its tie-break is not exact at 1e-9, so its likelihood is not yet a function of its parameters). Two caveats: the projection assumes each fit needs a comparable number of iterations, which scales with its own information H and is unmeasured elsewhere; and ~1 h of P11's run was dynesty's single-core unit-cube phase, which `first_update={'min_eff': 30}` would remove. `reports/P11_nested/task4_costs.csv`. |
| 1.18 | **Finish the second-seed reproducibility check** | PI | P11 started the prompt's second nested run (nlive 250, seed 2) at 04:04 and it had not converged by the morning — an unconverged nested log Z is a lower bound and cannot be compared, so the check is outstanding. Projected ~4,600 iterations and ~72,000 evaluations at nlive 250 (from the main run's H = 9.22), i.e. **3–4 h**, and its checkpoint is in place to resume. What is available meanwhile: dynesty's own volume-resampling check over 200 realisations gives log Z −22.925 ± 0.149 against the reported −22.913 ± 0.152 and posterior medians stable to 0.0004–0.05, which tests the estimator but not the sampling. `reports/P11_nested/task2_jitter.json`. |
