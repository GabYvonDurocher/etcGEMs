# Validation protocol for the revised target — SIGNED 2026-09-13

_Drafted 2026-09-13 by T1 as `VALIDATION_PROTOCOL_DRAFT.md`; **signed by the PI on 2026-09-13
with one amendment — check (d) — and frozen here by T2 (the commit after D0). No threshold below
may be revisited; a failed check is NOT PASSED for the programme.** `TARGET_REVISION_SPEC.md`
freezes the six required checks; this document supplies the quantitative thresholds it says a
signed follow-up must supply, and names where each came from and where that source stops applying.
RIGOUR.md rules 1, 2, 5, 6, 7, 8 and 10 govern its use. The draft is retained unchanged beside
this file (RIGOUR 4)._

**Applies to the approved revised target (T2):** configuration D NLDM with `f_metab` removed
(1.29), structural infeasibility at −∞ (1.30), no curvature correction (1.31), dTm pinned at 0;
five independent runs on reserved seeds 17901–17905 in the validation configuration (14 physical
+ 2 diagnostic coordinates).
...
### (a) Inactive-coordinate CDF recovery against its exact independent prior — SIGNED
...
- **Threshold (SIGNED):** weighted CDF distance **≤ 0.05** *and* |CDF mean − 0.5| **≤ 0.05**, on
...
### (b) Appended Beta(3,1) recovery judged on b³, with the wrong-Uniform contrast — SIGNED
...
- **Threshold (SIGNED):** correct-CDF distance **≤ 0.05**; wrong-CDF distance **≥ 0.30**; both must
...
### (c) Consistency of all active distributions and covariance directions across independent initialisations — SIGNED
...
- **Threshold (SIGNED):** all three, on all five runs.
...
### (d) AMENDED BY THE PI (2026-09-13): the PRIOR fraction rejected as infeasible, with its uncertainty — SIGNED

- **What changed and why:** under the approved −∞ rule the dead stratum (parameter sets
  infeasible at any measured temperature) has zero likelihood and is **excluded, not occupied**;
  its posterior mass is 0 by construction, so region occupancy no longer measures anything. The
  quantity that carries the same information — how much of a defensible prior the model rejects
  on NLDM — becomes the check, and item 1.33's finding.
- **Statistic:** (i) TASK 2's reference: 2,000 independent prior draws (development seed 17302),
  the fraction returning −∞, its Wilson 95 % interval, and the temperature at which rejection
  first fires in the registered order; (ii) per run: 2,000 fresh prior draws by the driver with
  `default_rng([seed, 1])`, the same fraction; (iii) per run: the count of posterior samples with
  finite importance weight that are infeasible at any temperature (must be 0), and the
  importance-weighted living fraction (peak predicted growth ≥ 50 % of 2.0761 /h), reported as a
  finding.
- **Threshold (SIGNED):** each run's fraction within **2 standard errors of the two-sample
  difference** from the reference (SE = √(p̂(1−p̂)(1/2000 + 1/2000)) with pooled p̂), on all five
  runs; and (iii)'s infeasible count **exactly 0** on all five runs. The living fraction has **no
  threshold** (RIGOUR: no biological living-fraction target).
- **Calibration:** binomial sampling only; no toy is involved. **Limit:** a fraction that agrees
  across runs certifies only that the prior and the feasibility boundary were the same in every
  run — the property (c) needs — not that the boundary is right (1.33 is the PI's question).
- **Uncertainty that accounts for ancestry** (the spec's wording) is carried by (c): five
  independent initialisations, each with its own root set.

> **The draft's (d), retained for the record (RIGOUR 4) and superseded by the amendment above:**
...
> - **Threshold (SIGNED):** region masses agreeing across the five runs to within **0.05**, *and* no
...
### (e) Feasibility-aware posterior predictive checks at every observed temperature, including the unresolved-evaluation rate — SIGNED
...
- **Threshold (SIGNED):** unresolved-evaluation rate **≤ 1 %** of draw-temperatures; every measured
...
- **Calibration:** **none exists** — P17 ran no feasibility-aware predictive check. The 1 % and
  ...
  (median predictive peak growth 0.0016 /h against 2.0761). **Limit:** un-calibrated; signed as
  proposed, the loosest bars that would have caught P16's failure.

### (f) Evidence and final-live-point reconstruction with independent hash audits — SIGNED
...
- **Threshold (SIGNED):** reconstructed log Z within **1e-9** of reported; weights normalising to
...
- **No region-occupancy threshold** (the draft's (d)); replaced by the prior-rejection fraction.
...
_Signed: **PI**, **2026-09-13**, with the (d) amendment — recorded verbatim in
`reports/T2_validated_posterior/DECISIONS.md` D0. Frozen at this commit; T2's five runs are
judged against these thresholds and no others._
